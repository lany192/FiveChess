...
class MonteCarloEngineTest {
    ...
    private fun engine(algorithm: AiAlgorithm, level: Difficulty = Difficulty.MEDIUM): GomokuAI =
        AiEngineFactory.create(algorithm, level, 15, 15, Random(11), clock = { 0L })
    ...
    fun `迷你棋盘也返回合法落点`() {
        ...
        for (algorithm in presets) {
            val point = AiEngineFactory.create(
                algorithm, Difficulty.MEDIUM, 5, 5, Random(11), clock = { 0L },
            ).getPosition(board)
