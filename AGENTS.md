...
- `data/net/` —— `Protocol.kt`（字节级编解码 + `TcpFrameReader`）、`LanDiscoveryManager`（UDP 发现/握手/聊天）、`LanGameClient`（TCP 对局）、`HeartbeatMonitor`（纯 Kotlin 心跳/假死检测，局域网与蓝牙共用）、`GameTransport`（对局传输抽象，`ui/net` 唯一依赖的建连接口）。阻塞式 `DatagramSocket.receive()` / `Socket.read()` 不响应协程取消 —— 须先关闭 socket 再取消 scope 来停止。
...
- TCP 端口 8899 对局：`[len][type][payload]`，len 为含自身的整帧总长；ADD_CHESS=0（payload `[x][y]`）、ROLLBACK_ASK=2 / ROLLBACK_AGREE=3 / ROLLBACK_REJECT=4、RESTART=5、DRAW_ASK=6 / DRAW_AGREE=7 / DRAW_REJECT=8、RESIGN=9、TIMEOUT=10、HEARTBEAT=11（空 payload）。
- 心跳/假死检测（`HeartbeatMonitor`，局域网/蓝牙共用）：连接后每 5s 发 HEARTBEAT，任意收帧刷新存活时间；**收到过对端心跳之后**才启用判死（≥15s 无帧 → 走既有 `Disconnected` 路径），旧版对端从不发心跳，因此只发不判、不误报。
...
- 对局页：两端用同一个 UUID 重新建连（服务端 `accept(30s)`、客户端 20 次 × 300ms 重试，覆盖对端换监听口的空窗）。帧格式完全复用 `Protocol.encodeTcp`/`TcpFrameReader`，因此悔棋/重开/求和/认输/心跳等语义自动与局域网一致（`HeartbeatMonitor` 覆盖两种传输）。
...
- 胜利判定为 4 个方向上连续五子；棋盘 15×15（`GameEngine` 默认）；**满盘（225 手）无五连即和棋**（五连优先；引擎发 `Draw`，联机两端各自从第 225 手本地推导，无新协议字节；旧版无此逻辑，满盘和棋要求两端同版本）。
- 协商请求**必须回执**：终局后禁发悔棋/求和，且收到请求一律回 REJECT（不静默丢包，否则对端永久悬挂）；"忙"（弹窗挂着/等待回应）时同样立即回 REJECT。
- 悔棋解除终局后比分**不回收**（三端一致口径）。
- 局域网联机页周期重播 JOIN（10s）+ 30s 未刷新自动淘汰离线设备；同名刷新原地更新不移动（防列表抖动）。
- 以上联机行为由 `ui/net/NetGameViewModel` 统一实现，**局域网与蓝牙必须保持一致** —— 要给联机加规则请改这里，不要在某一端单独加。

## 强制准则

- 必要注释：生成代码时需要，需要在关键位置添加必要的中文注释。
- 需求变更：任何需求变更都必须同步到需求文档DEMAND.md中，确保需求的透明性和可追溯性。
